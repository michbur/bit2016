...
@
    ...
    \begin{block}{Extraction of n-grams}
    \begin{figure}
    \centering
    \includegraphics[width=0.5\columnwidth]{static_figure/example.png} 
    \end{figure}

We extracted overlapping hexamers from all peptide 
sequences. Each hexamer was tagged with the same etiquette: amyloid (positive) 
or nonamyloid (negative) as the source peptide
...
<<echo = FALSE,message=FALSE,fig.align='center',fig.width=14,fig.height=20,warning=FALSE>>=
...
ggplot(ngram_freq_plot, aes(x = decoded_name, y = value)) +
  ...
  geom_point(data = group_by(ngram_freq_plot, decoded_name)  %>% filter(value == max(value)),
             aes(y = value + 0.002, shape = association), size = 6) +
